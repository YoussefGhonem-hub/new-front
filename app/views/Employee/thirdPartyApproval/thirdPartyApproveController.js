...
(function () {
    ...
    angular
        .module('eServices')
        .controller('thirdPartyApproveController', thirdPartyApproveController);

    thirdPartyApproveController.$inject = ['$rootScope', '$scope', 'UserProfile', '$filter', 'DTOptionsBuilder', 'DTColumnBuilder', '$compile', '$http', '$uibModal', '$state', 'SweetAlert'];
    function thirdPartyApproveController($rootScope, $scope, UserProfile, $filter, DTOptionsBuilder, DTColumnBuilder, $compile, $http, $uibModal, $state, SweetAlert) {
        var vm = this;
        ...
        vm.dtApplicationColumns = [
            ...
            DTColumnBuilder.newColumn('createdOn').withTitle(vm.translateFilter('dashboard.lastActionDate')).renderWith(
                function (data, type) {
                    return $filter('date')(data, 'dd-MMMM-yyyy');
                }),
        ...
        function serverData(sSource, aoData, fnCallback, oSettings) {
            ...
            $http.post($rootScope.app.httpSource + 'api/Application/GetPendingThirdPartyApprovalApplications', params)
                .then(function (resp) {
        ...
        function createdRow(row, data, dataIndex) {
            if (data.applicationDetails[0].payments.length > 0 && data.applicationDetails[0].applicationStatus.id != 4 && data.applicationDetails[0].applicationStatus.id != 10 && data.applicationDetails[0].applicationStatus.id != 11) {
