...
(function () {
    ...
    angular
        .module('eServices')
        .controller('managerApprovalController', managerApprovalController);

    managerApprovalController.$inject = ['$rootScope', '$scope', 'UserProfile', '$filter', 'DTOptionsBuilder', 'DTColumnBuilder', '$compile', '$http', '$uibModal', '$state', 'SweetAlert', '$window'];
    function managerApprovalController($rootScope, $scope, UserProfile, $filter, DTOptionsBuilder, DTColumnBuilder, $compile, $http, $uibModal, $state, SweetAlert, $window) {
        var vm = this;
        ...
        function serverData(sSource, aoData, fnCallback, oSettings) {
            ...
            if (vm.isTemporaryDelete === undefined) {
                $http.post($rootScope.app.httpSource + 'api/Application/GetPendingManagerApprovalApplications', vm.params)
                    .then(function (resp) {
        ...
        function format(dRow) {
            ...
            for (var i = 0; i < dRow.applicationDetails.length; i++) {
                ...
                html2 += '<tr role="row" class="bg-gray slider' + index + '"><td></td><td><b>' + vm.translateFilter('dashboard.mediaLicenseNumber') + ': ' +
                            dRow.applicationDetails[i].mediaLicenses[0].mediaLicenseNumber + '</b></td><td><b>' + vm.translateFilter('dashboard.applicationType') + ': ' +
        ...
        vm.filter = function ($scope) {
            ...
            vm.dtApplicationInstance._renderer.rerender();
        }
    }
})();
