...
(function () {
    ...
    angular
        .module('eServices')
        .controller('ReviewGroundPhotographyPermitController', ReviewGroundPhotographyPermitController);

    function ReviewGroundPhotographyPermitController($rootScope, $scope, $http, $stateParams, $state, $window, $uibModal, UserProfile, DTOptionsBuilder, DTColumnBuilder, $filter, $compile) {
        var vm = this;
        vm.translateFilter = $filter('translate');
        vm.dtPartnerInstance = {};
        vm.applicationOpen = true;
        vm.serviceFeesObj = { serviceId: 7, serviceFee: [] };
        ...
        vm.reviewLocation = function (size, locationId, event) {
            var modalInstance = $uibModal.open({
                templateUrl: 'app/views/Application/GroundPhotographyPermit/filmingLocation/Review/reviewFilmingLocation.html',
                ...
                resolve: {
                    location: function () {
                        ...
                        let location = $filter('filter')(vm.photographyPermitModel.photographyPermitLocations, { id: locationId }, true)[0];
                            if (!location.address.location) {
                                ...
                                if (location.address.location.geometry.wellKnownText.includes('POINT')) {
                                    Cordinates = location.address.location.geometry.wellKnownText.split('(')[1];
        ...
        vm.reviewFilmingTeamMember = function (size, teamMemberId, event) {
            var modalInstance = $uibModal.open({
                templateUrl: 'app/views/Application/GroundPhotographyPermit/filmingTeam/Review/reviewFilmingTeam.html',
    ...
    ReviewGroundPhotographyPermitController.$inject = ['$rootScope', '$scope', '$http', '$stateParams', '$state', '$window', '$uibModal', 'UserProfile', 'DTOptionsBuilder',
