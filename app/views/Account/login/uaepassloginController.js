/**=========================================================
 * Module: UAEPassLoginController.js
 * Controller for the Chat APP 
 =========================================================*/

(function () {
    'use strict';

    angular
        .module('eServices')
        .controller('UAEPassLoginController', UAEPassLoginController);
    function UAEPassLoginController($rootScope, $scope, LoginService, UserProfile, $state, SweetAlert, $http, $filter, RegisterService, rememberMeService, $stateParams) {
        var vm = this;
        var translate = $filter('translate');
        vm.currentStep = 0;
        vm.unverifieduser = false;
        vm.$rootScope = $rootScope;
        vm.emailAlreadyTaken = false;
        vm.UAEPassTiketId = '';
        vm.photo = "";
        vm.name = '';
        vm.email = '';
        vm.emailAlreadyTaken = false;
        vm.linkuserEmail = '';
        vm.linkuserPassword = '';
        vm.linkAccountError = '';
        vm.translateFilter = $filter('translate');
        vm.emiratesIDUsed = false;
        vm.emailUsed = false;


        vm.LinkUAEPassYes = function () {
            vm.currentStep = 3;
        };

        vm.LinkUAEPassNo = function () {
            //vm.currentStep = 4;
            vm.createNew();
        };

        vm.linkAccount = function () {
            vm.linkAccountError = '';
            vm.isBusy = true;
            $http.post($rootScope.app.httpSource + 'api/Account/LinkUAEPassAccount',
                {
                    UAEPassTiketId: vm.UAEPassTiketId,
                    linkuserEmail: vm.linkuserEmail,
                    linkuserPassword: vm.linkuserPassword
                }, { headers: { 'lang': $rootScope.language.selected === 'English' ? 'en' : 'ar' } })
                .then(function (response) {
      
                    vm.isBusy = false;

                    if (response.data.error === '') {
                        $state.go('page.login', { tkt: response.data.data });
                    } else {
                        vm.linkAccountError = vm.translateFilter('UAEPass.' + response.data.error);
                    }
                },
                    function (err) {
                        vm.isBusy = false;
                        $state.go('page.login');
                    });

        };

        vm.loginToAccount = function (userid) {
            $http.post($rootScope.app.httpSource + 'api/Account/ImpersonateUAEPass', { UAEPassTiketId: vm.UAEPassTiketId, userid: userid })
                .then(function (response) {
                    $state.go('page.login', { tkt: response.data });
                    vm.isBusy = false;
                },
                    function (response) {
                        vm.isBusy = false;
                        $state.go('page.login');
                    });
        };

        vm.createNew = function () {
            vm.emailAlreadyTaken = false;
            $http.get($rootScope.app.httpSource + 'api/Account/UAEPassNewAccount?tkt=' + vm.UAEPassTiketId,
                { headers: { 'lang': $rootScope.language.selected === 'English' ? 'en' : 'ar' } }
            ).then(function (response) {

                if (response.data.status == 1) {
                    $state.go('page.login', { tkt: response.data.data });
                } else if (response.data.status == 3) {
                    vm.emailAlreadyTaken = true;
                } else if (response.data.status == -1) {
                    var _err = translate(response.data.error);

                    SweetAlert.swal("error", _err, "error");
                };

            },
                function (error) {

                    var errorsts = error.data.status;

                    $state.go('page.login');
                });
        };
        // Base64 encoding service used by AuthenticationService
        var Base64 = {

            keyStr: 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/=',

            encode: function (input) {
                var output = "";
                var chr1, chr2, chr3 = "";
                var enc1, enc2, enc3, enc4 = "";
                var i = 0;

                do {
                    chr1 = input.charCodeAt(i++);
                    chr2 = input.charCodeAt(i++);
                    chr3 = input.charCodeAt(i++);

                    enc1 = chr1 >> 2;
                    enc2 = ((chr1 & 3) << 4) | (chr2 >> 4);
                    enc3 = ((chr2 & 15) << 2) | (chr3 >> 6);
                    enc4 = chr3 & 63;

                    if (isNaN(chr2)) {
                        enc3 = enc4 = 64;
                    } else if (isNaN(chr3)) {
                        enc4 = 64;
                    }

                    output = output +
                        this.keyStr.charAt(enc1) +
                        this.keyStr.charAt(enc2) +
                        this.keyStr.charAt(enc3) +
                        this.keyStr.charAt(enc4);
                    chr1 = chr2 = chr3 = "";
                    enc1 = enc2 = enc3 = enc4 = "";
                } while (i < input.length);

                return output;
            },

            decode: function (input) {
                var output = "";
                var chr1, chr2, chr3 = "";
                var enc1, enc2, enc3, enc4 = "";
                var i = 0;

                // remove all characters that are not A-Z, a-z, 0-9, +, /, or =
                var base64test = /[^A-Za-z0-9\+\/\=]/g;
                if (base64test.exec(input)) {
                    window.alert("There were invalid base64 characters in the input text.\n" +
                        "Valid base64 characters are A-Z, a-z, 0-9, '+', '/',and '='\n" +
                        "Expect errors in decoding.");
                }
                input = input.replace(/[^A-Za-z0-9\+\/\=]/g, "");

                do {
                    enc1 = this.keyStr.indexOf(input.charAt(i++));
                    enc2 = this.keyStr.indexOf(input.charAt(i++));
                    enc3 = this.keyStr.indexOf(input.charAt(i++));
                    enc4 = this.keyStr.indexOf(input.charAt(i++));

                    chr1 = (enc1 << 2) | (enc2 >> 4);
                    chr2 = ((enc2 & 15) << 4) | (enc3 >> 2);
                    chr3 = ((enc3 & 3) << 6) | enc4;

                    output = output + String.fromCharCode(chr1);

                    if (enc3 != 64) {
                        output = output + String.fromCharCode(chr2);
                    }
                    if (enc4 != 64) {
                        output = output + String.fromCharCode(chr3);
                    }

                    chr1 = chr2 = chr3 = "";
                    enc1 = enc2 = enc3 = enc4 = "";

                } while (i < input.length);

                return output;
            }
        };

        if (rememberMeService('7ZXYZ@L') && rememberMeService('UU@#90')) {
            vm.linkuserEmail = Base64.decode(rememberMeService('7ZXYZ@L'));
            vm.linkuserPassword = Base64.decode(rememberMeService('UU@#90'));
            vm.remember = true;
        }

        vm.rememberMe = function () {
            if (vm.remember) {
                rememberMeService('7ZXYZ@L', Base64.encode(vm.linkuserEmail).trim());
                rememberMeService('UU@#90', Base64.encode(vm.linkuserPassword).trim());
            } else {
                rememberMeService('7ZXYZ@L', '');
                rememberMeService('UU@#90', '');
            }
        };

        $scope.init = function () {


            var _status = $state.params.st;
            var _data = $state.params.dt;


            if (_status) {

                if (_status === '0') {
                    //vm.unverifieduser = true;
                    var __msg = $filter('translate')('UAEPass.unverifiedusertitle');
                    SweetAlert.swal(__msg);
                    $state.go('page.login')
                }
                else if (_status === '-1') {
                    vm.currentStep = 5;
                }
                else if (_status === '1') {
                    $state.go('page.login', { tkt: _data });
                }
                else if (_status === '2') {

                    vm.currentStep = 1;
                    vm.UAEPassTiketId = _data;

                    //$http.post($rootScope.app.httpSource + 'api/Account/GetUAEPassToken?tkt=' + $state.params.tkt)
                    //    .then(function (response) {
                    //        vm.currentStep = 1;

                    //        vm.photo = response.data.Photo;
                    //        vm.name = response.data.Name;
                    //        vm.email = response.data.Email;

                    //        vm.emiratesIDUsed = response.data.EmiratesIDUsed;
                    //        vm.emailUsed = response.data.EmailUsed;

                    //        vm.UAEPassTiketId = response.data.UAEPassTiketId;
                    //        if (response.data.lang) {
                    //            vm.$rootScope.language.set(response.data.lang);
                    //        }
                    //        vm.isBusy = false;
                    //    },
                    //        function (response) {
                    //            $state.go('page.login');
                    //        });
                }

            }
        };
    }



    UAEPassLoginController.$inject = ['$rootScope', '$scope', 'LoginService', 'UserProfile', '$state', 'SweetAlert', '$http', '$filter', 'RegisterService', 'rememberMeService', '$stateParams'];

})();