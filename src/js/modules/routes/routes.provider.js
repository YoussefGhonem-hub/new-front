...
(function () {
    ...
    function RouteProvider(VENDOR_ASSETS) {
        ...
        this.require = function () {
            var _args = arguments;
            return ['$ocLazyLoad', '$q', '$localStorage', function ($ocLL, $q, $localStorage) {
                ...
                function getRequired(name) {
                    if ($localStorage.settings != undefined && $localStorage.settings.layout && $localStorage.settings.layout.isRTL) {
                        if (VENDOR_ASSETS.modulesAr)
                            for (var m in VENDOR_ASSETS.modulesAr)
                                if (VENDOR_ASSETS.modulesAr[m].name && VENDOR_ASSETS.modulesAr[m].name === name)
                                    return VENDOR_ASSETS.modulesAr[m];
                    }
                    else {
