(function() {
    var ZXCVBN_SRC = 'vendor/passwordstrengthmeter/bower_components/zxcvbn/dist/zxcvbn.js';
                ...
                link: function ($scope, $element, $attrs, ngModelCtrl) {
                    ...
                    $element.on('blur change keydown', function (evt) {
                        ...
                        $scope.$evalAsync(function($scope) {
                            ...
                            //validate the given password with regular expression pattern
                            var regExpPattern = new RegExp('^(?=.*[a-z])(?=.*[A-Z])(?=.*[0-9])(?=.*[$@$!%*?&])[A-Za-z0-9$@$!%*?&]{7,10}');
                            var regExpValidate = regExpPattern.test(pwd);

                            // resolve password strength score using zxcvbn service
                            $scope.passwordStrength = pwd ? (regExpValidate && zxcvbn.score(pwd) || 0) : null;
