...
(function () {
    ...
    function Provider() {
        ...
        this.getErrorHTML = function (message) {
            return '<p class="validation-invalid" style="color: #f34235; font-size:11px">' + message + '</p>';
        };
        ...
        this.addMsgElement = function (element) {
            if (element[0].type == "radio") {
                return $(element[0].parentElement.parentElement).after('<span></span>');
            }
            else {
...
(function () {
    ...
    function Validator($injector, $filter) {
        var $validationProvider = $injector.get('$validation');
        ...
        var validFunc = function (element, validMessage, validation, scope, ctrl, attrs, param) {
            ...
            if (element[0].type == "radio") {
                messageElem = $(element[0].parentElement.parentElement.nextElementSibling);
            }
        ...
        var invalidFunc = function (element, validMessage, validation, scope, ctrl, attrs, param) {
            var messageToShow = validMessage || $filter('localizeString')($validationProvider.getDefaultMsg(validation).error);
            var invalidCallback = $parse(attrs.invalidCallback);
            var messageId = attrs.messageId;
            var validationGroup = attrs.validationGroup;
            var messageElem;

            if (messageId || validationGroup) {
                messageElem = angular.element(document.querySelector('#' + (messageId || validationGroup)));
            }
            else {
                messageElem = $validationProvider.getMsgElement(element);
            }
    ...
    Validator.$inject = ['$injector', '$filter'];
}).call(this);